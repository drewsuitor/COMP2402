...
public class FastDefaultList<T> extends AbstractList<T> {
	...
	class indexednode{
		Node u;
		int k;
		public indexednode(Node n, int j){
			u = n;
			k = j;
		}
		public Node getNode(){return u;}
	 ...
	 */
	protected indexednode findPred(int i) {
		...
		indexednode indNode = new indexednode(u, j);
		return indNode;
	}
	...
	public T get(int i) {
		...
		indexednode u = findPred(i);
		Node n = u.getNode();
		int k = u.getInt();

		if(n.next[0] != null && n.length[0] + k == i){
			return n.next[0].x;
		}
	...
	public static void main(String[] args) {
		...
		FastDefaultList<Integer> list = new FastDefaultList<Integer>();
		int size = 9;
		list.add(0, 1);
		list.add(1, 2);
		list.add(2, 3);
